...
def main(opt):
    ...
    with open(pred_file_path, 'r', encoding='utf-8') as fr:
        content = json.load(fr)
    
    for data_idx, content_line in enumerate(content):
        # src_l = content_line['text']
        src_l = content_line['title'] + ' '+ content_line['abstract']
        trg_list =  content_line['present_kp'] + content_line['absent_kp']
        trg_l = ";".join(trg_list)
        pred_list_tmp = list(content_line['predict'].values())
        pred_list = [item for sub_list in pred_list_tmp for item in sub_list]
        pred_l = ";".join(pred_list)
        ...
        tmp_src_l= src_l.strip().split('.')
        title = tmp_src_l[0]
        context = '.'.join(tmp_src_l[1:])
        ...
        if opt.export_filtered_pred:
            ...
            pred_print_out = re.sub(r'\ufe0f', '', pred_print_out) 
            pred_print_out = re.sub(r'\xae', '', pred_print_out)
            pred_print_out = re.sub(r'\xf1','', pred_print_out)
            pred_print_out = re.sub(r'\xf6','', pred_print_out)
...
if __name__ == '__main__':
    ...
    # qwen 70b chatgpt gpt4o deepseek
    data_name = 'duc'
    llm_name = 'llama'

    present_absent_segmenter = '<peos>'
    opt.pred_file_path = 'output/test/{}/{}/pred.json'.format(data_name, llm_name)
    opt.exp_path = 'output/test/{}/{}'.format(
        data_name, llm_name)
    # opt.exp_path = 'output/test/processed_results/kp20k/'
    opt.filtered_pred_path = 'output/test/{}/{}'.format(
        data_name, llm_name)

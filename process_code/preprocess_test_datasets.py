...
for name in ["duc", "openkp", "kptimes", "stackexchange", "kpbiomed"]:
    data_name = name
    RAW_CORPUS_FILE = os.path.join(os.getcwd(), "test_datasets", name, "test.json")
    if name =='kpbiomed':
        RAW_CORPUS_FILE = os.path.join(os.getcwd(), "test_datasets", name, "test.jsonl")     
    all_clean_texts = []
    ...
    output_file = os.path.join(os.getcwd(), "test_datasets", name, "test_clean.json")
    output_file_close = os.path.join(os.getcwd(), "test_datasets", name, "test_clean_close.json") 
    with open(output_file, "w") as f:
        json.dump(all_clean_texts[:2000], f, indent = 2)
    with open(output_file_close, "w") as f:
        json.dump(all_clean_texts[:300], f, indent = 2)        

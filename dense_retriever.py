    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if selector:
                ...
                _, out, _ = question_encoder(q_ids_batch, None, q_attn_mask)
...
def get_all_passages(ctx_sources):
    all_passages = {}
    with open(ctx_sources, 'r', encoding='utf-8') as fr:
        all_passages = json.load(fr)
...
def main(cfg: DictConfig):
    ...
    # qa_src.load_data()
    with open(cfg.qa_dataset, 'r', encoding='utf-8') as fr:
        qa_src = json.load(fr)
    
    total_queries = len(qa_src)
    for i in range(total_queries):
        qa_sample = qa_src[i]
        question = qa_sample['title'] + ' ' + qa_sample['abstract']
        answers = None
    ...
    retriever.index_encoded_data(cfg.encoded_ctx_files, index_buffer_sz, path_id_prefixes=None)
    # get top k results
    top_results_and_scores = retriever.get_top_docs(questions_tensor.numpy(), cfg.n_docs)
    res_dict = []
    for idx, item in enumerate(top_results_and_scores):
        res_dict.append(
            {
                'id':idx,
                'index':item[0],
                'scores':item[1].tolist()
            }
        )
    with open(cfg.out_file, 'w', encoding='utf-8') as fw:
        json.dump(res_dict, fw, indent=2)

...
class BiEncoderSample(object):
    ...
    domain: List[BiEncoderPassage]
    domain_passages: List[BiEncoderPassage]    


class JsonKGDataset(Dataset):
    def __init__(
        self,
        file: str,
        ...
        self.ood_file = ['/xxx/source_codes/valid_corpus.json']
        self.candidata_file = ['/xxx/source_codes/semantic_scores/candidate.json']
    ...
    def _load_all_data(self):
        ...
        candiate = read_data_from_json_files(self.candidata_file)
        ood_ = read_data_from_json_files(self.ood_file)
        # filter those without positive ctx
        self.data = [r for r in data if len(r["pos_index"]) > 0]
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        r = BiEncoderSample()
        r.query = self._process_query(json_sample["title"] + ' ' + json_sample["abstract"])
        positive_index = json_sample["pos_index"]
        positive_ctxs = []
        for idx in positive_index:
            positive_ctxs.append(self.candiate[idx]["title"] + ' ' + self.candiate[idx]["abstract"])
        # positive_ctxs = [json_sample["positive_index"]
        if self.exclude_gold:
            ctxs = [ctx for ctx in positive_ctxs if "score" in ctx]
            if ctxs:
                positive_ctxs = ctxs
        hard_negative_index = json_sample["neg_index"]
        hard_negative_ctxs = []
        for idx in hard_negative_index:
            hard_negative_ctxs.append(self.candiate[idx]["title"] + ' ' + self.candiate[idx]["abstract"])
        negative_ctxs = []
        ood_index = json_sample['ood_index']
        ood_ctxs = []
        domain_ctxs = []
        for idx in ood_index:
            ood_item = self.ood_[idx]
            domain = ood_item['domain']
            if domain == 'stackexchange' or domain =='openkp':
                text = ood_item['text']
            else:
                text = ood_item['title'] + ' ' + ood_item['abstract']
            domain_ctxs.append(domain)
            ood_ctxs.append(text)
        ...
        r.positive_passages = positive_ctxs # [create_passage(ctx) for ctx in positive_ctxs]
        r.negative_passages = negative_ctxs # [create_passage(ctx) for ctx in negative_ctxs]
        r.hard_negative_passages = hard_negative_ctxs # [create_passage(ctx) for ctx in hard_negative_ctxs]
        r.domain = domain_ctxs
        r.domain_passages = ood_ctxs

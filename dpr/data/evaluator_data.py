...
class JsonKGDataset(Dataset):
    ...
    def _load_all_data(self):
        ...
        self.data = data # [r for r in data if len(r["pos_index"]) > 0]
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        r = BiEncoderSample()
        r.query = self._process_query(json_sample["text"]) # + ' ' + json_sample["abstract"])
        ev_index = json_sample['neighbors']
        # positive_index = json_sample["pos_index"]
        ev_ctxs = []
        for idx in ev_index:
            ev_ctxs.append(self.candiate[idx]["title"] + ' ' + self.candiate[idx]["abstract"])
        r.ev_ctx = ev_ctxs
        kg = ';'.join(json_sample['generated_keyphrases'])
        r.kg = kg
        if 'f1_score' not in json_sample.keys():
            r.tgt_score = -1
        else:
            r.tgt_score = json_sample['f1_score']
        return r

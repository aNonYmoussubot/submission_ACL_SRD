...
if transformers.__version__.startswith("4"):
    from transformers import BertConfig, BertModel, RobertaModel, RobertaTokenizer, RobertaConfig
...
from dpr.models.evencoder import EvEncoder
...
def get_roberta_ev_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    
    sample_encoder = HFRobertaEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    evencoder = EvEncoder(sample_encoder, fix_ctx_encoder=fix_ctx_encoder)
    ...
    return tensorizer, evencoder, optimizer
...
class RobertaTensorizer:
    def __init__(self, tokenizer: RobertaTokenizer, max_length: int, pad_to_max: bool = True):
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.pad_to_max = pad_to_max
    ...
    def text_to_tensor(
        ...
        if len(token_ids) >= seq_len:
            token_ids = token_ids[0:seq_len] if apply_max_len else token_ids
            token_ids[-1] = self.tokenizer.eos_token_id  # RoBERTa uses </s> as the end token

        return torch.tensor(token_ids)

    def get_pair_separator_ids(self) -> T:
        return torch.tensor([self.tokenizer.eos_token_id])  # RoBERTa uses </s> as the separator
    ...
    def is_sub_word_id(self, token_id: int) -> bool:
        token = self.tokenizer.convert_ids_to_tokens([token_id])[0]
        return token.startswith("Ġ")  # RoBERTa uses Ġ to denote subword tokens

    def to_string(self, token_ids: Union[List[int], T], skip_special_tokens: bool = True) -> str:
        if isinstance(token_ids, T):

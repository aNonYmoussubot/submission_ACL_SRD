    ...
    def create_biencoder_input(
        ...
        for idx, sample in enumerate(samples):
            ...
            if shuffle and shuffle_positives:
                ...
                positive_ctx = sample.ev_ctx[:]
            # ood_ctx.extend(sample.domain_passages)
            domain_list.extend([idx] * len(positive_ctx))

            question = sample.query
            tgt_scores.append(sample.tgt_score)


            kg_ctxs = sample.kg

            kg_tensor = tensorizer.text_to_tensor(kg_ctxs, title=None)
 
            sample_ctxs_tensors = [
                tensorizer.text_to_tensor(ctx, title=None)
                for ctx in positive_ctx
            ]
            ...
            kg_tensors.append(kg_tensor)
           
        ctxs_tensor = torch.cat([ctx.view(1, -1) for ctx in ctx_tensors], dim=0)
        questions_tensor = torch.cat([q.view(1, -1) for q in question_tensors], dim=0)
        kg_tensors = torch.cat([o.view(1, -1) for o in kg_tensors], dim=0)
        tg_scores = torch.tensor(tgt_scores)

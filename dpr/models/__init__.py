...
def init_hf_roberta_evencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_roberta_ev_components

    return get_roberta_ev_components(args, **kwargs)
...
BIENCODER_INITIALIZERS = {
    ...
    'roberta_ev':init_hf_roberta_evencoder,
...
TENSORIZER_INITIALIZERS = {
    ...
    "roberta_ev": init_hf_roberta_tenzorizer,
...
def init_evaluator_components(encoder_type: str, args, **kwargs):
    return init_comp(BIENCODER_INITIALIZERS, encoder_type, args, **kwargs)

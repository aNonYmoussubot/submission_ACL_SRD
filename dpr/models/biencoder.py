...
class BiEncoder(nn.Module):
    ...
    def create_biencoder_input(
        ...
        for sample in samples:
            ...
            domain_list.extend(sample.domain)
            ...
            for ood_p in sample.domain_passages:
                sample_ood_tensor.append(tensorizer.text_to_tensor(ood_p, title=None))
            ood_ctx_tensors.extend(sample_ood_tensor)
            # ood_ctx_tensors.append(tensorizer.text_to_tensor(ood_p, title=None) for ood_p in sample.domain_passages)
       
        ctxs_tensor = torch.cat([ctx.view(1, -1) for ctx in ctx_tensors], dim=0)
        questions_tensor = torch.cat([q.view(1, -1) for q in question_tensors], dim=0)
        ood_ctx_tensors = torch.cat([o.view(1, -1) for o in ood_ctx_tensors], dim=0)

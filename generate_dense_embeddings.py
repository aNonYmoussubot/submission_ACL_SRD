...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        batch_token_tensors = [
            tensorizer.text_to_tensor(ctx[1], title=None) for ctx in batch
        ]
...
def main(cfg: DictConfig):
    ...
    with open(cfg.ctx_src, 'r', encoding='utf-8') as fr:
        ctx_src = json.load(fr)
    all_passages = []
    for idx, item in enumerate(ctx_src):
        all_passages.append((idx, item['title'] + ' ' + item['abstract']))

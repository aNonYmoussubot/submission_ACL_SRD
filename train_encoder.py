...
def compute_batch_domain_orth_loss(domain_emb_dict):

    domain_names = sorted(domain_emb_dict.keys())
    if len(domain_names) < 2:
        return torch.tensor(0.0, device=list(domain_emb_dict.values())[0].device)
    ...
    for i in range(len(domain_names)):
        for j in range(i + 1, len(domain_names)):
            embs_i = domain_emb_dict[domain_names[i]]
            embs_j = domain_emb_dict[domain_names[j]]
            loss_ij = compute_domain_orthogonality_loss(embs_i, embs_j)
            total_loss += loss_ij
            count += 1
    return total_loss / max(count, 1)

def compute_domain_orthogonality_loss(embs_1, embs_2):

    if embs_1.shape[0] <= 1 or embs_2.shape[0] <= 1:
        return torch.tensor(0.0, device=embs_1.device)
    mu_1 = embs_1.mean(dim=0)      # [d]
    mu_2 = embs_2.mean(dim=0)      # [d]
    var_1 = embs_1.var(dim=0)      # [d]
    var_2 = embs_2.var(dim=0)      # [d]
    mu_1_norm = mu_1 / (mu_1.norm(p=2) + 1e-9)
    mu_2_norm = mu_2 / (mu_2.norm(p=2) + 1e-9)
    var_1_norm = var_1 / (var_1.norm(p=2) + 1e-9)
    var_2_norm = var_2 / (var_2.norm(p=2) + 1e-9)
    dot_mean = torch.abs(torch.sum(mu_1_norm * mu_2_norm))
    dot_var  = torch.abs(torch.sum(var_1_norm * var_2_norm))
    return dot_mean + dot_var

def pairwise_distances_sq_l2(x, y=None):
    if y is None:
        y = x
    # x^2, y^2, then broadcast
    x_sq = (x**2).sum(dim=1, keepdim=True)  # [m,1]
    y_sq = (y**2).sum(dim=1, keepdim=True)  # [n,1]
    # pairwise dot product: expand for broadcast
    dist = x_sq + y_sq.T - 2*torch.matmul(x, y.T)
    return dist.clamp_min(0.0)  # numerical stability

def rbf_kernel_matrix(x, y=None, gamma=1.0):
    """
    Return RBF kernel matrix K where K[i,j] = exp(-gamma * ||x_i - y_j||^2).
    """
    dist_sq = pairwise_distances_sq_l2(x, y)
    return torch.exp(-gamma * dist_sq)

def compute_mmd(in_embs, out_embs, gamma=1.0):
    """
    in_embs: [m, d]
    out_embs: [n, d]
    ...
    if m < 2 or n < 2:
        # Edge case: if there's only 1 sample in a domain, MMD is not well-defined
        return torch.tensor(0.0, device=in_embs.device)
    # Kxx: [m, m], Kyy: [n, n], Kxy: [m, n]
    Kxx = rbf_kernel_matrix(in_embs, in_embs, gamma=gamma)
    Kyy = rbf_kernel_matrix(out_embs, out_embs, gamma=gamma)
    Kxy = rbf_kernel_matrix(in_embs, out_embs, gamma=gamma)
    # sum over i != j by subtracting diagonal
    # sum_{i != j} k(s_i, s_j) = Kxx.sum() - diagonal
    sum_Kxx = Kxx.sum() - Kxx.diagonal().sum()
    sum_Kyy = Kyy.sum() - Kyy.diagonal().sum()
    # MMD^2 formula
    mmd = (sum_Kxx / (m * (m - 1))) + \
          (sum_Kyy / (n * (n - 1))) - \
          2 * (Kxy.sum() / (m * n))
...
def _calc_distribution_shift(local_q_vector, local_ood_vectors, domain_emb_dict, gamma=1.0):
    mmd_loss = compute_mmd(local_q_vector, local_ood_vectors, gamma=gamma)
    domain_orth_loss = compute_batch_domain_orth_loss(domain_emb_dict)
    return mmd_loss, domain_orth_loss
...
def _calc_loss(
    ...
    contrastive_loss, is_correct = loss_function.calc(
        global_q_vector,
        global_ctxs_vector,
        positive_idx_per_question,
        hard_negatives_per_question,
    ...
    return contrastive_loss, is_correct
...
def _get_ood_embed(ood_vectors, ood_list):
    domain_emb_buckets = defaultdict(list)
    for i, domain_name in enumerate(ood_list):
        domain_emb_buckets[domain_name].append(ood_vectors[i])

    domain_emb_dict = {}
    for d_name, emb_list in domain_emb_buckets.items():
        domain_emb_dict[d_name] = torch.stack(emb_list, dim=0)  # [N_d, hidden_size]
    return domain_emb_dict
...
def _do_biencoder_fwd_pass(
    ...
    ood_attn_mask = tensorizer.get_attn_mask(input.ood_ctx_tensors)
    ...
    _, local_ood_vectors, _ = model.question_model(
        ood_inputs,
        None,
        ood_attn_mask,
    ...
    domain_emb_dict = _get_ood_embed(local_ood_vectors, domain_list)

    loss_function = BiEncoderNllLoss()

    contrastive_loss, is_correct = _calc_loss(
        cfg,
        loss_function,
        local_q_vector,
        local_ctx_vectors,
    ...
    mmd_loss, domain_loss= _calc_distribution_shift(local_q_vector, local_ood_vectors, domain_emb_dict)

    loss = contrastive_loss + 0.2 * mmd_loss + 0.1 * domain_loss
    if cfg.n_gpu > 1:
        loss = loss.mean()
